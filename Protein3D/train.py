...
from equivariant_attention.utils_profiling import * # load before other local modules
import argparse
import sys
import warnings
...
def train_epoch(epoch, model, loss_fnc, dataloader, optimizer, scheduler, FLAGS):
    ...
    for i, (g, y) in enumerate(dataloader):
        if i == 0:
            print(f"g: {g}")
        #     print(f"g.ndata['f']: {g.ndata['f'].shape, g.ndata['f']}")
        #     print(f"g.ndata['x']: {g.ndata['x'].shape, g.ndata['x']}")
        #     print(f"g.edata['d']: {g.edata['d'].shape, g.edata['d']}")
        #     print(f"g.edata['w']: {g.edata['w'].shape, g.edata['w']}")
        # print(f"y: {y}")

        # if i == 0:
        #     save_graphs("./dloader0.bin", [g], {'glabel': torch.tensor([0])})
...
#     print(FLAGS.data_address)
    ...
    model = models.__dict__.get(FLAGS.model)(FLAGS.num_layers, 
                                             ...
                                             FLAGS.num_channels,                # feature types
                                             num_nlayers=FLAGS.num_nlayers,
                                             num_degrees=FLAGS.num_degrees,     # #l = lmax-1
                                             edge_dim=3,
                                             n_bonds=train_dataset.num_bonds-1,
                                             div=FLAGS.div,
